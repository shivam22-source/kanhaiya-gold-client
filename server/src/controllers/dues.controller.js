...
function normalizeBranch(branch) { return String(branch || '').trim(); }
function parseAmount(value) { const amount = Number(value); return Number.isFinite(amount) ? amount : NaN; }
...
export async function createDue(req, res, next) {
  // Backwards-compatible endpoint; normal certificates now create dues automatically.
  try {
    ...
    const certificateResult = await pool.query(
      `SELECT id, ref_no, borrower_name, certificate_date, item_image_url,
       ...
       FROM certificates WHERE id = $1`, [certificateId]);
    ...
    if (!Number.isFinite(amount) || amount <= 0) return res.status(400).json({ message: 'This certificate has no appraisal charge to track.' });
    const result = await pool.query(
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE certificate_dues
       SET due_amount = $1, status = 'active', updated_at = NOW()
...
export async function recordPayment(req, res, next) {
  ...
  try {
    ...
    const dueResult = await client.query(
      `SELECT id, due_amount, status FROM certificate_dues WHERE certificate_id = $1 FOR UPDATE`, [certificateId]);
    ...
    if (amount > currentDue) { await client.query('ROLLBACK'); return res.status(400).json({ message: `Payment cannot exceed the current due of ₹${currentDue.toFixed(2)}.` }); }

    const paymentResult = await client.query(
      `INSERT INTO due_payments (due_id, amount, paid_at) VALUES ($1, $2, NOW())
       RETURNING id, due_id, amount, paid_at, created_at`, [due.id, amount]);
    const updatedDue = await client.query(
      `UPDATE certificate_dues
       SET due_amount = due_amount - $1,
           ...
           updated_at = NOW()
       WHERE id = $2
       RETURNING id, certificate_id, initial_due, due_amount, status, created_at, updated_at`, [amount, due.id]);
...
export async function deleteDue(req, res, next) {
  try {
    ...
    if (!certificateId) return res.status(400).json({ message: 'Certificate ID is required.' });
    const result = await pool.query(
      `UPDATE certificate_dues SET status = 'closed', updated_at = NOW()
       WHERE certificate_id = $1 AND due_amount = 0
       RETURNING id, certificate_id`, [certificateId]);
    if (result.rowCount) return res.json({ deleted: true, certificateId });
    const existing = await pool.query('SELECT due_amount, status FROM certificate_dues WHERE certificate_id = $1', [certificateId]);
