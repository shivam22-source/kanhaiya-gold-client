...
function canAccess(user, ownerId) {
  return user.role === 'admin' || String(user.id) === String(ownerId);
}
...
export async function createCertificate(req, res, next) {
  ...
  try {
    ...
    const result = await client.query(
      `INSERT INTO certificates (borrower_name, ref_no, certificate_date, item_image_url, total_market_value, payload, user_id)
       VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
      [form.borrowerName, form.refNo || null, form.date || null, shop.itemImageUrl || null, Number(totals.marketValue) || 0, payload, req.user.id],
    );
    ...
    if (appraisalCharge > 0) {
      await client.query(
        `INSERT INTO certificate_dues (certificate_id, initial_due, due_amount, status)
         VALUES ($1, $2, $2, 'active')
         ON CONFLICT (certificate_id) DO UPDATE SET initial_due = EXCLUDED.initial_due,
           due_amount = EXCLUDED.due_amount, status = 'active', updated_at = NOW()`,
...
export async function listCertificates(req, res, next) {
  try {
    ...
    if (search) {
      params.push(`%${search}%`);
      conditions.push(`(borrower_name ILIKE $${params.length} OR ref_no ILIKE $${params.length})`);
    }
...
export async function getCertificate(req, res, next) {
  try {
    ...
    if (!canAccess(req.user, row.user_id)) return res.status(403).json({ message: 'You do not have access to this certificate.' });
...
export async function deleteCertificate(req, res, next) {
  ...
  try {
    await client.query('BEGIN');
    const targetResult = await client.query(`SELECT id, ref_no, user_id FROM certificates WHERE id = $1 FOR UPDATE`, [req.params.id]);
    if (!targetResult.rowCount) { await client.query('ROLLBACK'); return res.status(404).json({ message: 'Certificate not found.' }); }
    const target = targetResult.rows[0];
    if (!canAccess(req.user, target.user_id)) { await client.query('ROLLBACK'); return res.status(403).json({ message: 'You do not have access to this certificate.' }); }

    const pattern = '^KJ[-\\s]?(\\d+)$';
    const latestResult = await client.query(
      `SELECT id, ref_no, (regexp_match(ref_no, $1, 'i'))[1]::int AS serial_no
       FROM certificates WHERE ref_no ~* $1 ORDER BY serial_no DESC LIMIT 1 FOR UPDATE`,
      [pattern],
    );
    ...
    await client.query(
      `INSERT INTO deleted_certificate_payment_history (certificate_id, ref_no, payment_amount, paid_at)
       SELECT d.certificate_id, $2, p.amount, p.paid_at
       FROM due_payments p JOIN certificate_dues d ON d.id = p.due_id WHERE d.certificate_id = $1`,
...
export async function getNextSerial(req, res, next) {
  try {
    ...
    const ownerCondition = req.user.role === 'admin' ? '' : ' AND user_id = $2';
    if (req.user.role !== 'admin') params.push(req.user.id);
    const result = await pool.query(`SELECT MAX((regexp_match(ref_no, $1))[1]::int) AS max_no FROM certificates WHERE ref_no ~* $1${ownerCondition}`, params);
