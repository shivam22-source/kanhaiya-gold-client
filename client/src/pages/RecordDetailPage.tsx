...
function RecordDetailPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!cancelled) {
          setRecord({ ...result, isLatest: currentSerial > 0 && currentSerial === maxSerial });
  ...
  async function handleDelete() {
    ...
    const confirmed = window.confirm(`Delete latest certificate ${record.refNo || ''} for ${record.borrowerName}?\n\nOnly the latest certificate can be deleted. The next serial will reuse this number.`);
    if (!confirmed) return;
    try {
      setDeleting(true);
  ...
  const fallbackInitialDue = Math.max(Number(form.appraisalCharge) || 0, 0);
  const initialDue = dueSummary ? Number(dueSummary.initialDue || 0) : fallbackInitialDue;
    ...
    <main className="min-h-screen bg-slate-50 pb-24 text-slate-800">
      <header className="border-b border-slate-200 bg-white px-4 py-4 sm:px-6">
        <div className="mx-auto flex max-w-[1300px] flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div><button className="text-xs font-semibold uppercase tracking-wide text-indigo-600" onClick={() => navigate('/records')}>&larr; Back to Records</button><h1 className="mt-1 text-xl font-bold text-slate-950 sm:text-2xl">{record.borrowerName}</h1></div>
          <div className="grid grid-cols-1 gap-2 sm:flex"><button className="h-11 rounded bg-indigo-600 px-4 text-sm font-semibold text-white shadow-sm transition hover:bg-indigo-700 disabled:opacity-60" onClick={handleRegenerate} disabled={deleting}>Regenerate PDF</button>{record.isLatest && <button className="h-11 rounded border border-red-200 bg-white px-4 text-sm font-semibold text-red-600 shadow-sm transition hover:bg-red-50 disabled:opacity-60" onClick={handleDelete} disabled={deleting}>{deleting ? 'Deleting...' : 'Delete Latest Record'}</button>}</div>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-[1300px] space-y-5 p-3 sm:p-5">
        ...
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
          <Card title="Shop Details"><InfoRow label="Shop Name" value={shop.nameHindi} /><InfoRow label="Shop Address" value={shop.addressHindi} /><InfoRow label="Registration No." value={shop.registrationNo} /><InfoRow label="Appraiser A/c No." value={shop.appraiserAccount} />{shop.itemImageUrl && <div className="mt-3 flex items-center gap-3"><img className="h-16 w-16 rounded border border-slate-200 object-cover" src={shop.itemImageUrl} alt="Gold item" /><a className="text-xs font-semibold text-indigo-600" href={shop.itemImageUrl} target="_blank" rel="noreferrer">Open Photo</a></div>}</Card>
          <Card title="Reference, Bank & Borrower Details"><InfoRow label="Ref No." value={form.refNo} /><InfoRow label="Appraisal Charge (Rs.)" value={form.appraisalCharge ? formatMoney(form.appraisalCharge) : '-'} /><InfoRow label="Date" value={form.date} /><InfoRow label="Bank A/c No." value={form.bankAccount} /><InfoRow label="Branch Name" value={form.branchName} /><InfoRow label="Borrower Name" value={form.borrowerName} /><InfoRow label="Father/Husband Name" value={form.fatherName} /><InfoRow label="Borrower Resident" value={form.borrowerAddress} /><InfoRow label="Appraisal Date" value={form.appraisalDate} /><InfoRow label="Cash-in-charge" value={form.cashInCharge} /><InfoRow label="Testing Method" value={form.testingMethod} /><InfoRow label="Place" value={form.place} /><InfoRow label="Signature Date" value={form.signatureDate} /></Card>
        </div>
